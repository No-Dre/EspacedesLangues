...
setInterval(function(){
    tmp_val = Game.ObjectsById[0].price / Game.ObjectsById[0].storedCps;
    pos = 0;
    for (let i = 1; i < Game.ObjectsById.length; i++){
        if (tmp_val > Game.ObjectsById[i].price / Game.ObjectsById[i].storedCps){
            if(Game.ObjectsById[i].price > Game.cookies){
                if ((Game.ObjectsById[i].price - Game.cookies) / Game.cookiesPs < (Game.ObjectsById[i].price + Game.ObjectsById[pos].price - Game.cookies) / (Game.cookiesPs + Game.ObjectsById[pos].storedCps)){
                    tmp_val = Game.ObjectsById[i].price / Game.ObjectsById[i].storedCps;
                    pos = i
                }
            } else {
                tmp_val = Game.ObjectsById[i].price / Game.ObjectsById[i].storedCps;
    ...
    if (Game.ObjectsById[pos].price <= Game.cookies){
        Game.ObjectsById[pos].buy()
    }
}, 3);
